...
const INTRO_PARTICLE_COLOR = 'rgb(255, 179, 48)'
...
class AnimationController {
    ...
    public showProjectedDot(position: Vector3D, sizeFactor: number) {
        ...
        if (position.z > newCameraZ) {
            ...
            const radius = this.constrain(sw * 0.68, 0.62, 1.55)
    ...
    public render() {
        ...
        // 绘制星星
        ctx.fillStyle = INTRO_PARTICLE_COLOR
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            const sw = (1.45 * (1 - t1) + 3.35 * Math.sin(Math.PI * t1)) * f

            this.ctx.fillStyle = INTRO_PARTICLE_COLOR
            this.ctx.lineWidth = sw
...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            const dotSize = 9.4 * this.strokeWeightFactor * sizeMultiplier;
