...
export function ParticleTextEffect({
  ...
  const getWordFont = (word: string, canvas: HTMLCanvasElement) => {
    const widthFit = canvas.width / Math.max(word.length * 0.66, 7)
    const heightFit = canvas.height * 0.18
    const size = Math.max(52, Math.min(136, widthFit, heightFit))

    return `bold ${size}px "SD Dystopian", "Share Tech Mono", Arial, sans-serif`
  }
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    offscreenCtx.font = getWordFont(word, canvas)
  ...
  const animate = () => {
    ...
    if (mouseRef.current.isPressed && mouseRef.current.isRightClick) {
      const scatterRadius = Math.max(68, Math.min(116, canvas.width * 0.06))
  ...
  useEffect(() => {
    ...
    const syncCanvasSize = () => {
      ...
      const nextWidth = Math.max(360, Math.round(rect?.width || 1000))
      const nextHeight = Math.max(500, Math.round(rect?.height || 500))
    ...
    // The original demo initializes the canvas scene once and owns its RAF loop.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  return (
    <div ref={rootRef} className="particle-page">
      <canvas
        ref={canvasRef}
